...
async function sendAdmin(){const b=document.getElementById('customerAiSendAdmin');b.disabled=true;const {data:{user}}=await client.auth.getUser();if(!user){add('bot','로그인 후 문의를 전달할 수 있습니다.');b.disabled=false;return}const {data:p}=await client.from('customers').select('business_name,owner_name').eq('id',user.id).maybeSingle();const {error}=await client.from('customer_ai_inquiries').insert({customer_id:user.id,customer_name:p?.business_name||p?.owner_name||'',question:lastUnknown,status:'대기'});if(error){add('bot','문의 전달에 실패했습니다. V6.6.4 SQL 설치 여부를 확인해주세요.');b.disabled=false;return}add('bot','관리자에게 문의를 전달했습니다. 답변이 등록되면 이 문의창의 이전 문의에서 확인할 수 있습니다.');b.hidden=true}
...
document.body.insertAdjacentHTML('beforeend',`<button id="customerAiButton" class="customer-ai-button" type="button">💬 AI 문의</button><div id="customerAiModal" class="customer-ai-modal" hidden><section><header><div><strong>무료 AI 문의</strong><small>기본 질문을 누르거나 직접 입력하세요</small></div><button type="button" data-ai-close>×</button></header><div id="customerAiMessages" class="customer-ai-messages"><div class="customer-ai-message bot"><p>아래 기본 질문을 누르면 바로 답변해드립니다.</p></div><div class="customer-ai-quick-questions">${quickQuestions.map((x,i)=>`<button type="button" data-ai-quick="${i}"><b>${i+1}.</b> ${esc(x[0])}</button>`).join('')}</div></div><button id="customerAiSendAdmin" type="button" class="customer-ai-admin-send" hidden>관리자에게 문의 전달</button><button id="customerAiHistoryBtn" type="button" class="customer-ai-history-btn">이전 문의·답변 보기</button><div id="customerAiHistory" class="customer-ai-history" hidden></div><footer><textarea id="customerAiInput" rows="2" placeholder="직접 질문을 입력할 수도 있습니다."></textarea><button id="customerAiAsk" type="button">보내기</button></footer><p class="customer-ai-disclaimer">자동답변은 안내용입니다. 주문·금액의 최종 정보는 주문조회 화면을 확인해주세요.</p></section></div>`);
const modal=document.getElementById('customerAiModal'),openAi=()=>modal.hidden=false;document.getElementById('customerAiButton').onclick=openAi;document.querySelectorAll('[data-ai-quick]').forEach(button=>button.onclick=()=>{const row=quickQuestions[Number(button.dataset.aiQuick)];if(row)askQuick(row[0],row[1])});const bottomNav=document.querySelector('.customer-bottom-nav');if(bottomNav){bottomNav.classList.add('has-ai-menu');const link=document.createElement('a');link.href='#ai-inquiry';link.className='customer-ai-nav-link';link.innerHTML='<span>💬</span>AI문의';link.onclick=e=>{e.preventDefault();openAi()};bottomNav.appendChild(link)}modal.querySelector('[data-ai-close]').onclick=()=>modal.hidden=true;document.getElementById('customerAiAsk').onclick=ask;document.getElementById('customerAiSendAdmin').onclick=sendAdmin;document.getElementById('customerAiHistoryBtn').onclick=history;document.getElementById('customerAiInput').onkeydown=e=>{if(e.key==='Enter'&&!e.shiftKey){e.preventDefault();ask()}};
})();
