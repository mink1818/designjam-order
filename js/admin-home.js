...
async function loadUnpaidCustomers(){
  ...
  try{
    const [orders,payments]=await Promise.all([fetchDashboardRows('orders','order_number,customer_id,customer_name,qty,price,soldout_qty,is_soldout,shipping_fee,created_at','created_at'),fetchDashboardRows('order_payment_records','order_number,customer_key,paid_amount,confirmed_by,updated_at','updated_at')]);
