...
async function loadUnpaidCustomers(){
  ...
  try{
    ...
    const paymentMap=new Map();payments.forEach(x=>{const paid=Number(x.paid_amount||0);paymentMap.set(`${x.order_number}::${x.customer_key||''}`,paid);paymentMap.set(`order::${x.order_number}`,paid)});const orderMap=new Map();
    orders.forEach(row=>{const name=row.customer_name||'거래처 미입력',nameKey=String(name).normalize('NFKC').replace(/[\s_.·ㆍ,()[\]{}\-/]+/g,'').toLowerCase(),key=`${row.order_number}::${nameKey||row.customer_id||''}`;if(!orderMap.has(key))orderMap.set(key,{orderNumber:row.order_number,customerId:String(row.customer_id||''),name,total:0,shipping:0});const g=orderMap.get(key),ordered=Number(row.qty||0),soldout=Math.min(ordered,Number(row.soldout_qty||(row.is_soldout?ordered:0)));g.total+=Math.max(0,ordered-soldout)*Number(row.price||0);g.shipping=Math.max(g.shipping,Number(row.shipping_fee||0))});
    const customerMap=new Map();let unpaidOrders=0,unpaidTotal=0;
    orderMap.forEach((g,key)=>{const storedPaid=paymentMap.has(key)?paymentMap.get(key):paymentMap.get(`order::${g.orderNumber}`);if(storedPaid===undefined)return;const total=g.total+g.shipping,paid=Math.max(0,storedPaid||0),balance=Math.max(0,total-paid);if(balance<=0)return;unpaidOrders++;unpaidTotal+=balance;const ck=String(g.name||'거래처 미입력').normalize('NFKC').replace(/\s+/g,'').toLowerCase();if(!customerMap.has(ck))customerMap.set(ck,{...g,count:0,balance:0});const c=customerMap.get(ck);c.count++;c.balance+=balance});
