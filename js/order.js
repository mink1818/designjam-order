...
function downloadOrderCsv(){if(!myOrderGroups.length){alert('저장할 주문내역이 없습니다.');return;}const rows=[['주문일','주문번호','상태','품번','수량(죽)','단가(1죽)','금액','택배사','송장번호']];myOrderGroups.forEach(g=>g.items.forEach(i=>rows.push([formatDate(g.createdAt),g.orderNumber,g.status||'',String(i.item_number||'').replace(/^[SBI](?:[-_\s]+|(?=\d))/i,''),i.qty,i.price,Number(i.price||0)*Number(i.qty||0),g.courier||'',g.trackingNumber||''])));const csv='\uFEFF'+rows.map(r=>r.map(v=>'"'+String(v??'').replaceAll('"','""')+'"').join(',')).join('\r\n');const blob=new Blob([csv],{type:'text/csv;charset=utf-8'});const a=document.createElement('a');a.href=URL.createObjectURL(blob);a.download=`디자인 삭스_주문내역_${new Date().toISOString().slice(0,10)}.csv`;a.click();setTimeout(()=>URL.revokeObjectURL(a.href),1000);}
...
async function deletePendingOrder(orderNumber, editing=false){
  ...
  const pickingStarted=group.items.some(item=>Number(item.picked_qty||0)>0||Number(item.soldout_qty||0)>0||item.picking_session_active===true||Boolean(item.picking_started_at)||!["","대기"].includes(String(item.picking_status||"대기")));
  if(pickingStarted)return alert('피킹을 시작했거나 피킹완료된 주문은 수정하거나 삭제할 수 없습니다. 관리자에게 문의해주세요.');
  const allowed=await supabaseClient.rpc('assert_customer_order_revision_allowed',{p_order_number:orderNumber});if(allowed.error)return alert(allowed.error.message);
