...
function hideRestrictedMenus(allowed){document.querySelectorAll('a,button,[data-link]').forEach(element=>{const target=managerTarget(element);if(target&&!allowed.has(target)){element.hidden=true;element.classList.add('manager-restricted-menu');element.style.setProperty('display','none','important')}});document.querySelectorAll('.global-admin-search,.v3-dashboard-section:last-of-type,.unpaid-customer-panel').forEach(element=>{element.hidden=true;element.style.setProperty('display','none','important')})}
function applyManagerDashboardMetrics(){if((location.pathname.split('/').pop()||'')!=='admin-home.html')return;const keep=new Set(['todayOrderCount','pendingOrderCount','todayDoneCount']);document.querySelectorAll('.v3-metric-card').forEach(card=>{const strong=card.querySelector('strong');if(!strong||!keep.has(strong.id)){card.hidden=true;card.style.setProperty('display','none','important')}else{card.hidden=false;card.style.removeProperty('display')}})}
function hideEmployeeDashboardMetrics(){if((location.pathname.split('/').pop()||'')!=='admin-home.html')return;const grid=document.querySelector('.v3-metric-grid');if(grid){grid.hidden=true;grid.style.setProperty('display','none','important')}}
...
async function enforce(){const page=location.pathname.split('/').pop()||'admin.html';if(document.body?.dataset?.sessionPage==='customer')return;const p=await loadRole();if(p===undefined)return;if(!p){if(page!=='admin.html')location.replace('admin.html');return}if(!p.is_admin||p.blocked){location.replace('admin.html');return}document.documentElement.dataset.adminRole=p.admin_role||'admin';if(p.admin_role==='manager'){hideManagerUnauthorizedMenus();applyManagerBarcodeOnly(page);applyManagerDashboardMetrics();setTimeout(()=>{hideManagerUnauthorizedMenus();applyManagerBarcodeOnly(page);applyManagerDashboardMetrics()},100);if(!MANAGER_ALLOWED.has(page)){alert('매니저 계정은 허용된 업무 화면만 사용할 수 있습니다.');location.replace('admin-home.html');return}}else if(p.admin_role==='employee'){hideRestrictedMenus(EMPLOYEE_ALLOWED);hideEmployeeDashboardMetrics();setTimeout(()=>{hideRestrictedMenus(EMPLOYEE_ALLOWED);hideEmployeeDashboardMetrics()},100);if(!EMPLOYEE_ALLOWED.has(page)){alert('직원 계정은 허용된 작업 화면만 사용할 수 있습니다.');location.replace('picking.html');return}}else if(page!=='ai-inquiries.html'&&!document.querySelector('script[data-ai-inquiry-alert]')){const script=document.createElement('script');script.src='js/admin-ai-inquiry-alert.js?v=66040';script.dataset.aiInquiryAlert='1';document.body.appendChild(script)}}
