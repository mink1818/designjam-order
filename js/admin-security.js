...
function hideManagerUnauthorizedMenus(){document.querySelectorAll('a,button,[data-link]').forEach(element=>{const target=managerTarget(element);if(target&&!MANAGER_ALLOWED.has(target))element.hidden=true});document.querySelectorAll('.global-admin-search,.v3-metric-grid,.v3-dashboard-section:last-of-type').forEach(element=>element.hidden=true)}
