...
async function fetchSavedProxyParties(){
 const parties=await fetchAll('admin_proxy_parties','id,customer_name,normalized_name,linked_customer_id,last_used_at','customer_name');
...
function directCustomerRows(){
 ...
 proxySavedParties.forEach(party=>{const name=String(party.customer_name||'').trim(),key=normalizeCustomer(name);if(name&&!rows.has(key))rows.set(key,{name,customerId:String(party.linked_customer_id||''),partyId:String(party.id),owner:'',phone:'',kind:party.linked_customer_id?'가입 거래처':'미가입·저장 거래처'})});
...
function renderProxyCustomerSuggestions(query=''){const box=$('proxyCustomerSuggestions');if(!box)return;const value=String(query||'').trim();if(!value){box.hidden=true;box.innerHTML='';return}const rows=directCustomerRows().filter(row=>lookupMatches([row.name,row.owner,row.phone].filter(Boolean).join(' '),value)).sort((a,b)=>String(a.name).localeCompare(String(b.name),'ko',{numeric:true})).slice(0,40);box.hidden=false;box.innerHTML=rows.length?rows.map((row,index)=>`<button type="button" data-party-index="${index}"><b>${esc(row.name)}</b><small>${esc(row.kind)}${row.owner?` · ${esc(row.owner)}`:''}</small></button>`).join(''):'<p>검색 결과가 없습니다. 입력한 이름으로 새 거래처를 사용할 수 있습니다.</p>';box.querySelectorAll('[data-party-index]').forEach(button=>button.onclick=()=>selectUnifiedProxyParty(rows[Number(button.dataset.partyIndex)]))}
...
async function submit(){
 ...
 try{
  ...
  if(mode==='direct'){
   ...
   if(savedParty.error)throw new Error(`직접입력 거래처·납품처 목록 저장 실패: ${savedParty.error.message} · V6.5.69 SQL을 먼저 실행해주세요.`);
  }
