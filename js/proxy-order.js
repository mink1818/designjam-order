...
// V6.6.60: 저장 납품처 선택값을 주문 접수까지 별도로 고정한다.
...
function selectProxyDelivery(row){if(!row)return;proxyDeliveryManuallyEdited=false;selectedProxyDeliverySnapshot={id:String(row.id),delivery_name:row.delivery_name||'',delivery_phone:row.delivery_phone||'',delivery_address:row.delivery_address||''};$('proxyDeliveryName').value=row.delivery_name||'';$('proxyDeliveryPhone').value=row.delivery_phone||'';$('proxyDeliveryAddress').value=row.delivery_address||'';$('proxyDeliverySelect').value=String(row.id);$('proxyDeliverySuggestions').hidden=true;updateProxyDestinationActions();scheduleProxyDraftSave()}
...
async function submit(){
 ...
 const lockedDestination=(selectedProxyDeliverySnapshot&&proxyDestinations.some(row=>String(row.id)===selectedProxyDeliverySnapshot.id))?selectedProxyDeliverySnapshot:null;
 const finalSelectedDestination=selectedDestination||lockedDestination;
