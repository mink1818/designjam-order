...
function renderProxyCustomerSuggestions(query=''){const box=$('proxyCustomerSuggestions');if(!box)return;const value=String(query||'').trim();if(!value){box.hidden=true;box.innerHTML='';return}const rows=directCustomerRows().filter(row=>lookupMatches([row.name,row.owner,row.phone].filter(Boolean).join(' '),value)).sort((a,b)=>String(a.name).localeCompare(String(b.name),'ko',{numeric:true})).slice(0,40);box.hidden=false;box.innerHTML=rows.length?rows.map((row,index)=>`<div class="proxy-party-suggestion-row"><button type="button" class="proxy-party-select" data-party-index="${index}"><b>${esc(row.name)}</b><small>${esc(row.kind)}${row.owner?` · ${esc(row.owner)}`:''}</small></button>${row.partyId&&!row.customerId?`<button type="button" class="proxy-party-delete" data-delete-party-id="${esc(row.partyId)}" data-delete-party-name="${esc(row.name)}">삭제</button>`:''}</div>`).join(''):'<p>검색 결과가 없습니다. 입력한 이름으로 새 거래처를 사용할 수 있습니다.</p>';box.querySelectorAll('[data-party-index]').forEach(button=>button.onclick=()=>selectUnifiedProxyParty(rows[Number(button.dataset.partyIndex)]));box.querySelectorAll('[data-delete-party-id]').forEach(button=>button.onclick=event=>{event.stopPropagation();deleteSavedProxyParty(button.dataset.deletePartyId,button.dataset.deletePartyName)})}
