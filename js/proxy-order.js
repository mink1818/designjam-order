...
function saveProxyDraft(){const key=proxyDraftKey();if(!key||draftSubmissionComplete)return;const draft={memo:$('proxyMemo')?.value||'',rows:proxyDraftRows(),saved_at:new Date().toISOString()};localStorage.setItem(key,JSON.stringify(draft))}
