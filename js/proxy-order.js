...
function renderProxyCustomerSuggestions(query=''){const box=$('proxyCustomerSuggestions');if(!box)return;const value=String(query||'').trim();if(!value){box.hidden=true;box.innerHTML='';return}const rows=customers.filter(customer=>lookupMatches([customer.business_name,customer.owner_name,customer.email].filter(Boolean).join(' '),value)).sort((a,b)=>String(a.business_name||a.owner_name||'').localeCompare(String(b.business_name||b.owner_name||''),'ko',{numeric:true})).slice(0,30);box.hidden=false;box.innerHTML=rows.length?rows.map(c=>`<button type="button" data-customer-id="${c.id}"><b>${esc(c.business_name||c.owner_name||'거래처')}</b><small>${esc(c.owner_name||'')} · ${esc(c.phone||c.email||'')}</small></button>`).join(''):'<p>검색된 거래처가 없습니다.</p>';box.querySelectorAll('[data-customer-id]').forEach(button=>button.onclick=()=>selectProxyCustomer(button.dataset.customerId))}
function selectProxyCustomer(id){const customer=customers.find(c=>String(c.id)===String(id));if(!customer)return;renderProxyCustomerOptions(customer.business_name||customer.owner_name||'');$('proxyCustomer').value=String(customer.id);$('proxyCustomerSearch').value=customer.business_name||customer.owner_name||'';$('proxySelectedCustomerLabel').textContent=`선택됨: ${customer.business_name||customer.owner_name||customer.email}`;$('proxyCustomerSuggestions').hidden=true;$('proxyCustomer').dispatchEvent(new Event('change'))}
function renderProxyDestinationOptions(customer,query=''){const select=$('proxyDeliverySelect');if(!select)return;const selected=select.value;const rows=proxyDestinations.filter(row=>lookupMatches([row.delivery_name,row.delivery_phone,row.delivery_address].filter(Boolean).join(' '),query)).sort((a,b)=>String(a.delivery_name||'').localeCompare(String(b.delivery_name||''),'ko',{numeric:true}));select.innerHTML='<option value="registered">등록 주소 사용</option>'+rows.map(row=>`<option value="${row.id}">${esc(row.delivery_name)}${row.is_default?' · 기본':''}</option>`).join('')+'<option value="new">+ 새 납품처 입력</option>';if(selected==='registered'||selected==='new'||rows.some(row=>String(row.id)===selected))select.value=selected;else select.value='registered'}
...
function resetProxyParties(){if($('proxySelectedCustomerLabel'))$('proxySelectedCustomerLabel').textContent='선택된 거래처 없음';const selectMode=document.querySelector('input[name="proxyCustomerMode"][value="select"]');if(selectMode)selectMode.checked=true;clearProxyPartyFields()}
...
async function applyPastedOrder(){
 ...
 if(fields.customer){const matched=customers.find(c=>normalizeCustomer(c.business_name)===normalizeCustomer(fields.customer));if(matched){document.querySelector('input[name="proxyCustomerMode"][value="select"]').checked=true;renderProxyCustomerOptions(fields.customer);$('proxyCustomer').value=matched.id}else{document.querySelector('input[name="proxyCustomerMode"][value="direct"]').checked=true;$('proxyDirectName').value=fields.customer}updateCustomerMode();await reloadSelectedCustomerPrices()}
...
async function loadProxyDestinations(preferredFields=null){const customerId=selectedCustomerId(),select=$('proxyDeliverySelect');if(!select)return[];proxyDestinations=[];renderProxyDestinationOptions(null);if(!customerId)return[];const customer=customers.find(c=>String(c.id)===String(customerId));const {data,error}=await supabaseClient.from('customer_delivery_destinations').select('id,delivery_name,delivery_phone,delivery_address,is_default,last_used_at').eq('customer_id',customerId).order('is_default',{ascending:false}).order('last_used_at',{ascending:false}).limit(500);if(error)throw error;proxyDestinations=data||[];const nameList=$('proxyDeliveryNameList');if(nameList)nameList.innerHTML=proxyDestinations.sort((a,b)=>String(a.delivery_name||'').localeCompare(String(b.delivery_name||''),'ko',{numeric:true})).map(row=>`<option value="${esc(row.delivery_name)}">${esc(row.delivery_address||row.delivery_phone||'저장 납품처')}</option>`).join('');renderProxyDestinationOptions(customer,'');const applySelection=()=>{if(proxyDeliveryManuallyEdited&&!preferredFields)return;const row=proxyDestinations.find(x=>String(x.id)===select.value);if(select.value==='registered'){$('proxyDeliveryName').value=customer?.business_name||'';$('proxyDeliveryPhone').value=customer?.phone||'';$('proxyDeliveryAddress').value=customer?.address||'';}else if(select.value==='new'){if(!preferredFields){$('proxyDeliveryPhone').value='';$('proxyDeliveryAddress').value='';}}else if(row){$('proxyDeliveryName').value=row.delivery_name||'';$('proxyDeliveryPhone').value=row.delivery_phone||'';$('proxyDeliveryAddress').value=row.delivery_address||'';}scheduleProxyDraftSave()};select.onchange=()=>{proxyDeliveryManuallyEdited=false;preferredFields=null;applySelection()};const matched=findMatchingProxyDestination(proxyDestinations,preferredFields||{});if(proxyDeliveryManuallyEdited)select.value='new';else if(matched)select.value=String(matched.id);else if(preferredFields&&(preferredFields.deliveryName||preferredFields.deliveryPhone||preferredFields.deliveryAddress))select.value='new';else select.value='registered';applySelection();return proxyDestinations}
...
async function init(){
 ...
 try{
  const [customerRows,inventoryRows,groups]=await Promise.all([fetchAll('customers','id,business_name,owner_name,email,phone,address,approved,blocked,is_admin','created_at'),fetchAll('inventory_items','*','item_number'),fetchAll('product_groups','*','sort_order')]);
...
$('proxyCustomerSearch')?.addEventListener('input',event=>{renderProxyCustomerOptions(event.target.value);renderProxyCustomerSuggestions(event.target.value)});
$('proxyCustomerSearch')?.addEventListener('change',event=>{const matches=customers.filter(customer=>lookupMatches([customer.business_name,customer.owner_name,customer.email].filter(Boolean).join(' '),event.target.value));if(matches.length===1)selectProxyCustomer(matches[0].id)});
...
$('proxyDeliveryName')?.addEventListener('input',()=>{proxyDeliveryManuallyEdited=true;if($('proxyDeliverySelect'))$('proxyDeliverySelect').value='new';scheduleProxyDraftSave()});
