...
// V6.6.61: 저장 납품처 선택값을 거래처 UUID와 함께 주문 접수까지 별도로 고정한다.
let selectedProxyDeliverySnapshot=null;
let selectedProxyDeliveryCustomerId='';
...
function lookupRank(label,query){const needle=normalizeCustomer(query);if(!needle)return 0;const source=normalizeCustomer(label),initials=normalizeCustomer(koreanInitialText(label));if(source===needle)return 0;if(source.startsWith(needle))return 1;if(initials===needle)return 2;if(initials.startsWith(needle))return 3;if(source.includes(needle))return 4;if(initials.includes(needle))return 5;return 99}
function customerSearchCompare(nameA,nameB,query){const rank=lookupRank(nameA,query)-lookupRank(nameB,query);return rank||String(nameA||'').localeCompare(String(nameB||''),'ko',{numeric:true})}
...
function customerMetaText(customer){const parts=[];if(customer?.customer_code)parts.push(`[${customer.customer_code}]`);if(customer?.owner_name)parts.push(customer.owner_name);if(customer?.phone)parts.push(customer.phone);return parts.join(' · ')}
function directCustomerRows(){
 ...
 customers.forEach(customer=>{const name=String(customer.business_name||customer.owner_name||customer.email||'').trim();if(name)rows.push({name,customerId:String(customer.id),owner:customer.owner_name||'',phone:customer.phone||'',code:customer.customer_code||'',kind:'가입 거래처'})});
 const registeredNames=new Set(customers.map(c=>normalizeCustomer(c.business_name||c.owner_name||c.email)));
 proxySavedParties.forEach(party=>{const name=String(party.customer_name||'').trim(),key=normalizeCustomer(name);if(name&&!party.is_hidden&&!registeredNames.has(key))rows.push({name,customerId:String(party.linked_customer_id||''),partyId:String(party.id),owner:'',phone:'',code:'',kind:party.linked_customer_id?'가입 거래처':'미가입·저장 거래처'})});
 proxyPartyHistory.forEach(row=>{const name=String(row.customer_name||'').trim(),key=normalizeCustomer(name);if(name&&!hiddenNames.has(key)&&!registeredNames.has(key)&&!rows.some(x=>!x.customerId&&normalizeCustomer(x.name)===key))rows.push({name,customerId:'',owner:'',phone:'',code:'',kind:'미가입·이전 주문'})});
 return rows;
}
function renderDirectCustomerSuggestions(query=''){
 ...
 const rows=directCustomerRows().filter(row=>lookupMatches(row.name,query)).sort((a,b)=>customerSearchCompare(a.name,b.name,query)).slice(0,40);
 box.hidden=false;box.innerHTML=rows.length?rows.map((row,index)=>`<button type="button" data-direct-index="${index}"><b>${esc(row.name)}</b><small>${esc(row.kind)}${row.code?` · [${esc(row.code)}]`:''}${row.owner?` · ${esc(row.owner)}`:''}${row.phone?` · ${esc(row.phone)}`:''}</small></button>`).join(''):'<p>일치하는 거래처가 없습니다. 새 거래처명으로 입력할 수 있습니다.</p>';
...
function renderProxyCustomerOptions(query=''){const select=$('proxyCustomer');if(!select)return;const selected=select.value;const rows=customers.filter(customer=>lookupMatches([customer.business_name,customer.owner_name,customer.email,customer.phone,customer.customer_code].filter(Boolean).join(' '),query)).sort((a,b)=>customerSearchCompare(a.business_name||a.owner_name||'',b.business_name||b.owner_name||'',query));select.innerHTML='<option value="">거래처 선택</option>'+rows.map(c=>`<option value="${c.id}">${esc(c.business_name||c.owner_name||'거래처')} · ${esc(customerMetaText(c)||c.email||String(c.id).slice(0,8))}</option>`).join('');if(rows.some(row=>String(row.id)===selected))select.value=selected}
function selectNewProxyPartyName(name){
 const value=String(name||'').trim();if(!value)return;
 document.querySelector('input[name="proxyCustomerMode"][value="direct"]').checked=true;
 $('proxyCustomer').value='';$('proxyDirectName').value=value;$('proxyDirectOwner').value='';$('proxyDirectPhone').value='';$('proxyDirectAddress').value='';
 $('proxyCustomerSearch').value=value;$('proxySelectedCustomerLabel').textContent=`새 미가입 거래처: ${value}`;
 $('proxyCustomerSuggestions').hidden=true;proxyDeliveryManuallyEdited=false;
 proxyDestinations=[];renderProxyDestinationOptions(null);$('proxyDeliveryName').value='';$('proxyDeliveryPhone').value='';$('proxyDeliveryAddress').value='';
 loadProxyDestinations();reloadSelectedCustomerPrices();calc();scheduleProxyDraftSave();
}
function renderProxyCustomerSuggestions(query=''){
 const box=$('proxyCustomerSuggestions');if(!box)return;
 const value=String(query||'').trim();if(!value){box.hidden=true;box.innerHTML='';return}
 const rows=directCustomerRows().filter(row=>lookupMatches([row.name,row.owner,row.phone,row.code].filter(Boolean).join(' '),value)).sort((a,b)=>customerSearchCompare(a.name,b.name,value)).slice(0,40);
 const hasExactName=directCustomerRows().some(row=>normalizeCustomer(row.name)===normalizeCustomer(value));
 box.hidden=false;
 box.innerHTML=(hasExactName?'':`<button type="button" class="proxy-party-new" data-new-party-name="1"><b>${esc(value)}</b><small>이 이름으로 직접 입력</small></button>`)+rows.map((row,index)=>`<div class="proxy-party-suggestion-row"><button type="button" class="proxy-party-select" data-party-index="${index}"><b>${esc(row.name)}</b><small>${esc(row.kind)}${row.code?` · [${esc(row.code)}]`:''}${row.owner?` · ${esc(row.owner)}`:''}${row.phone?` · ${esc(row.phone)}`:''}</small></button>${row.partyId&&!row.customerId?`<button type="button" class="proxy-party-delete" data-delete-party-id="${esc(row.partyId)}" data-delete-party-name="${esc(row.name)}">삭제</button>`:''}</div>`).join('');
 box.querySelector('[data-new-party-name]')?.addEventListener('click',()=>selectNewProxyPartyName(value));
 box.querySelectorAll('[data-party-index]').forEach(button=>button.onclick=()=>selectUnifiedProxyParty(rows[Number(button.dataset.partyIndex)]));
 box.querySelectorAll('[data-delete-party-id]').forEach(button=>button.onclick=event=>{event.stopPropagation();deleteSavedProxyParty(button.dataset.deletePartyId,button.dataset.deletePartyName)});
}
...
function selectProxyDelivery(row){if(!row)return;proxyDeliveryManuallyEdited=false;selectedProxyDeliverySnapshot={id:String(row.id),delivery_name:row.delivery_name||'',delivery_phone:row.delivery_phone||'',delivery_address:row.delivery_address||''};selectedProxyDeliveryCustomerId=selectedCustomerId();$('proxyDeliveryName').value=row.delivery_name||'';$('proxyDeliveryPhone').value=row.delivery_phone||'';$('proxyDeliveryAddress').value=row.delivery_address||'';$('proxyDeliverySelect').value=String(row.id);$('proxyDeliverySuggestions').hidden=true;updateProxyDestinationActions();scheduleProxyDraftSave()}
...
function saveProxyDraft(){const key=proxyDraftKey();if(!key||draftSubmissionComplete)return;const draft={memo:$('proxyMemo')?.value||'',admin_tag:$('proxyAdminTag')?.value||'',rows:proxyDraftRows(),saved_at:new Date().toISOString()};localStorage.setItem(key,JSON.stringify(draft))}
...
function clearProxyPartyFields(){['proxyCustomer','proxyDirectName','proxyDirectOwner','proxyDirectPhone','proxyDirectAddress','proxyDeliveryName','proxyDeliveryPhone','proxyDeliveryAddress','proxyCustomerSearch','proxyDeliverySearch'].forEach(id=>{if($(id))$(id).value=''});if($('proxyDeliverySelect'))$('proxyDeliverySelect').innerHTML='<option value="">거래처 선택 후 불러오기</option>';proxyDestinations=[];proxyDeliveryManuallyEdited=false;selectedProxyDeliverySnapshot=null;selectedProxyDeliveryCustomerId='';activeCustomerPrices.clear();selectedPriceLoadToken++;updateRegisteredPriceStatus('');if($('proxyDirectPriceMatch'))$('proxyDirectPriceMatch').hidden=true}
...
async function loadProxyDestinations(preferredFields=null){
 ...
 select.onchange=()=>{proxyDeliveryManuallyEdited=false;preferredFields=null;const chosen=proxyDestinations.find(x=>String(x.id)===String(select.value));selectedProxyDeliverySnapshot=chosen?{id:String(chosen.id),delivery_name:chosen.delivery_name||'',delivery_phone:chosen.delivery_phone||'',delivery_address:chosen.delivery_address||''}:null;selectedProxyDeliveryCustomerId=chosen?selectedCustomerId():'';applySelection();updateProxyDestinationActions()};
...
async function submit(){
 ...
 // 저장 목록에서 고른 납품처는 화면 재렌더링으로 select가 흔들려도 마지막 명시 선택값을 우선한다.
 const activeCustomerId=selectedCustomerId();
 const lockedDestination=(selectedProxyDeliverySnapshot&&selectedProxyDeliveryCustomerId===activeCustomerId&&proxyDestinations.some(row=>String(row.id)===selectedProxyDeliverySnapshot.id))?selectedProxyDeliverySnapshot:null;
 // 사용자가 저장 목록에서 명시적으로 고른 값은 렌더링 과정에서 select가 기본 납품처로 돌아가도 절대 거래처명으로 덮어쓰지 않는다.
 const finalSelectedDestination=lockedDestination||selectedDestination;
 if(finalSelectedDestination){
   $('proxyDeliveryName').value=finalSelectedDestination.delivery_name||'';
   $('proxyDeliveryPhone').value=finalSelectedDestination.delivery_phone||'';
   $('proxyDeliveryAddress').value=finalSelectedDestination.delivery_address||'';
...
async function init(){
 ...
 try{
  const [customerRows,inventoryRows,groups,partyHistory,savedDirectory,metaRows]=await Promise.all([fetchAll('customers','id,business_name,owner_name,email,phone,address,approved,blocked,is_admin','created_at'),fetchAll('inventory_items','*','item_number'),fetchAll('product_groups','*','sort_order'),fetchProxyPartyHistory(),fetchSavedProxyParties().catch(()=>({parties:[],destinations:[]})),fetchAll('customer_admin_metadata','customer_id,customer_code,customer_tag,show_order_tag','updated_at').catch(()=>[])]);
  const metaMap=new Map((metaRows||[]).map(m=>[String(m.customer_id),m]));customers=customerRows.filter(x=>!x.is_admin&&!x.blocked).map(x=>({...x,...(metaMap.get(String(x.id))||{})}));
...
$('proxyCustomerSearch')?.addEventListener('blur',()=>setTimeout(()=>{const box=$('proxyCustomerSuggestions');if(box)box.hidden=true},200));
$('proxyCustomerSearch')?.addEventListener('keydown',event=>{if(event.key==='Escape'){$('proxyCustomerSuggestions').hidden=true;event.target.blur()}else if(event.key==='Enter'){event.preventDefault();const name=String(event.target.value||'').trim();if(!name)return;const exact=directCustomerRows().find(row=>normalizeCustomer(row.name)===normalizeCustomer(name));if(exact)selectUnifiedProxyParty(exact);else selectNewProxyPartyName(name);event.target.blur()}});
document.addEventListener('pointerdown',event=>{if(!event.target.closest?.('#proxySelectWrap')){const box=$('proxyCustomerSuggestions');if(box)box.hidden=true}});
...
$('confirmProxyPaste')?.addEventListener('click',()=>{if(!selectedProxyPhotoFiles.length||!window.FreeHandwritingOCR?.saveTrainingData)return;const files=[...selectedProxyPhotoFiles];setTimeout(()=>{const confirmed=[...document.querySelectorAll('.proxy-line')].map(row=>({item_number:String(row.querySelector('.proxy-item')?.value||'').trim(),qty:Math.max(1,Number(row.querySelector('.proxy-qty')?.value||1))})).filter(row=>row.item_number);if(confirmed.length)window.FreeHandwritingOCR.saveTrainingData(supabaseClient,files,confirmed,'admin').catch(error=>console.warn('학습자료 저장 생략:',error.message))},300)});
