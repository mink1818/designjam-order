...
function clearProxyPartyFields(){['proxyCustomer','proxyDirectName','proxyDirectOwner','proxyDirectPhone','proxyDirectAddress','proxyDeliveryName','proxyDeliveryPhone','proxyDeliveryAddress','proxyCustomerSearch','proxyDeliverySearch'].forEach(id=>{if($(id))$(id).value=''});if($('proxyDeliverySelect'))$('proxyDeliverySelect').innerHTML='<option value="">거래처 선택 후 불러오기</option>';proxyDestinations=[];activeCustomerPrices.clear();selectedPriceLoadToken++;updateRegisteredPriceStatus('');if($('proxyDirectPriceMatch'))$('proxyDirectPriceMatch').hidden=true}
function resetProxyParties(){const selectMode=document.querySelector('input[name="proxyCustomerMode"][value="select"]');if(selectMode)selectMode.checked=true;clearProxyPartyFields()}
