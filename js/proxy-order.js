...
function saveProxyDraft(){const key=proxyDraftKey();if(!key||draftSubmissionComplete)return;const mode=document.querySelector('input[name="proxyCustomerMode"]:checked')?.value||'select';const draft={mode,customer_id:$('proxyCustomer')?.value||'',direct_name:$('proxyDirectName')?.value||'',direct_owner:$('proxyDirectOwner')?.value||'',direct_phone:$('proxyDirectPhone')?.value||'',direct_address:$('proxyDirectAddress')?.value||'',delivery_name:$('proxyDeliveryName')?.value||'',delivery_phone:$('proxyDeliveryPhone')?.value||'',delivery_address:$('proxyDeliveryAddress')?.value||'',memo:$('proxyMemo')?.value||'',paste:$('proxyPasteInput')?.value||'',rows:proxyDraftRows(),saved_at:new Date().toISOString()};localStorage.setItem(key,JSON.stringify(draft))}
...
async function init(){
 ...
 try{
  ...
  const preset=new URLSearchParams(location.search).get('customer');if(preset){$('proxyCustomer').value=preset;$('proxyCustomer').dispatchEvent(new Event('change'))}if(!restoreProxyDraft())addLine();await reloadSelectedCustomerPrices();calc();
...
document.getElementById('proxyCustomer')?.addEventListener('change',()=>{const customer=customers.find(c=>String(c.id)===String($('proxyCustomer').value));if(customer){$('proxyDeliveryName').value=customer.business_name||'';$('proxyDeliveryPhone').value=customer.phone||'';$('proxyDeliveryAddress').value=customer.address||'';}loadProxyDestinations();scheduleProxyDraftSave()});
$('proxyCustomerSearch')?.addEventListener('input',event=>renderProxyCustomerOptions(event.target.value));
$('proxyCustomerSearch')?.addEventListener('change',event=>{const matches=customers.filter(customer=>lookupMatches([customer.business_name,customer.owner_name,customer.email].filter(Boolean).join(' '),event.target.value));if(matches.length===1){renderProxyCustomerOptions(event.target.value);$('proxyCustomer').value=matches[0].id;$('proxyCustomer').dispatchEvent(new Event('change'))}});
...
$('proxyDeliveryName')?.addEventListener('change',event=>{const query=String(event.target.value||'').trim();const exact=proxyDestinations.find(row=>normalizeCustomer(row.delivery_name)===normalizeCustomer(query));const initialMatches=proxyDestinations.filter(row=>lookupMatches(row.delivery_name,query));const matched=exact||(initialMatches.length===1?initialMatches[0]:null);if(matched){$('proxyDeliverySelect').value=String(matched.id);$('proxyDeliverySelect').dispatchEvent(new Event('change'))}else{$('proxyDeliverySelect').value='new';$('proxyDeliveryPhone').value='';$('proxyDeliveryAddress').value='';scheduleProxyDraftSave()}});
})();
