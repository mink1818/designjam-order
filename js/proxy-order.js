...
function saveProxyDraft(){const key=proxyDraftKey();if(!key||draftSubmissionComplete)return;const draft={memo:$('proxyMemo')?.value||'',admin_tag:$('proxyAdminTag')?.value||'',admin_tag_on:$('proxyAdminTagOn')?.checked===true,rows:proxyDraftRows(),saved_at:new Date().toISOString()};localStorage.setItem(key,JSON.stringify(draft))}
...
async function submit(){
 ...
 try{
  ...
  const adminTag=String($('proxyAdminTag')?.value||'').trim(),showAdminTag=$('proxyAdminTagOn')?.checked===true;
  if(showAdminTag||adminTag){const tagSave=await supabaseClient.from('order_admin_metadata').upsert({order_number:order,admin_tag:adminTag,show_tag:showAdminTag&&Boolean(adminTag),updated_at:new Date().toISOString()},{onConflict:'order_number'});if(tagSave.error)throw new Error(`관리자 표시 저장 실패: ${tagSave.error.message} · V6.6.20-ORDER-ADMIN-TAG.sql을 먼저 실행해주세요.`)}
