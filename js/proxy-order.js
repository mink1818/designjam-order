...
let customers=[],items=[],proxyPartyHistory=[],currentAdminId=null,activeCustomerPrices=new Map(),selectedPriceLoadToken=0,directPriceTimer=null;
...
function directCustomerRows(){
 ...
 proxyPartyHistory.forEach(row=>{const name=String(row.customer_name||'').trim(),key=normalizeCustomer(name);if(name&&!rows.has(key))rows.set(key,{name,customerId:'',owner:'',phone:'',kind:'미가입·이전 주문'})});
 return [...rows.values()];
}
...
function renderProxyDestinationOptions(customer,query=''){const select=$('proxyDeliverySelect');if(!select)return;const selected=select.value;const rows=proxyDestinations.filter(row=>lookupMatches([row.delivery_name,row.delivery_phone,row.delivery_address].filter(Boolean).join(' '),query)).sort((a,b)=>String(a.delivery_name||'').localeCompare(String(b.delivery_name||''),'ko',{numeric:true}));select.innerHTML=(customer?'<option value="registered">거래처 기본 납품처</option>':'')+rows.map(row=>`<option value="${esc(row.id)}">${esc(row.delivery_name)}${row.is_default?' · 기본':''}</option>`).join('')+'<option value="new">+ 새 납품처 입력</option>';if(selected==='registered'||selected==='new'||rows.some(row=>String(row.id)===selected))select.value=selected;else select.value=customer?'registered':(rows[0]?String(rows[0].id):'new')}
...
function selectProxyDelivery(row){if(!row)return;proxyDeliveryManuallyEdited=false;$('proxyDeliveryName').value=row.delivery_name||'';$('proxyDeliveryPhone').value=row.delivery_phone||'';$('proxyDeliveryAddress').value=row.delivery_address||'';$('proxyDeliverySelect').value=String(row.id);$('proxyDeliverySuggestions').hidden=true;scheduleProxyDraftSave()}
...
async function loadProxyDestinations(preferredFields=null){
 ...
 else {const directName=normalizeCustomer($('proxyDirectName')?.value);const unique=new Map();proxyPartyHistory.filter(row=>normalizeCustomer(row.customer_name)===directName&&String(row.delivery_name||'').trim()).forEach((row,index)=>{const key=normalizeDeliveryLookup(row.delivery_name)+'|'+normalizeDeliveryLookup(row.delivery_address);if(!unique.has(key))unique.set(key,{id:`history-${index}`,delivery_name:row.delivery_name,delivery_phone:row.delivery_phone||'',delivery_address:row.delivery_address||'',is_default:false,last_used_at:row.created_at})});proxyDestinations=[...unique.values()]}
...
async function init(){
 ...
 try{
  const [customerRows,inventoryRows,groups,partyHistory]=await Promise.all([fetchAll('customers','id,business_name,owner_name,email,phone,address,approved,blocked,is_admin','created_at'),fetchAll('inventory_items','*','item_number'),fetchAll('product_groups','*','sort_order'),fetchProxyPartyHistory()]);
