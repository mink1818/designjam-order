...
function applyPastedOrder(){const text=String($('proxyPasteInput')?.value||'').trim();if(!text)return alert('붙여넣을 주문정보를 입력하세요.');const rows=text.split(/\r?\n|[;|]/).map(line=>line.trim()).filter(Boolean),merged=new Map(),unmatched=[];const fields={};const labels=[['customer',/^(?:거래처명?|업체명?)\s*[:：]\s*(.+)$/i],['delivery',/^(?:납품처명?|배송처명?)\s*[:：]\s*(.+)$/i],['phone',/^(?:연락처|전화(?:번호)?)\s*[:：]\s*(.+)$/i],['address',/^(?:주소|납품주소|배송주소)\s*[:：]\s*(.+)$/i],['memo',/^(?:메모|요청사항)\s*[:：]\s*(.+)$/i]];const addParsed=(item,qty)=>{if(!item)return;const key=normalizeItem(item);const current=merged.get(key)||{item_number:item,qty:0};current.qty+=qty;merged.set(key,current)};for(const line of rows){let handled=false;for(const [key,re] of labels){const match=line.match(re);if(match){fields[key]=match[1].trim();handled=true;break}}if(handled)continue;const cells=line.split(/\t+/).map(v=>v.trim()).filter(Boolean);if(cells.length>=2){const qtyText=String(cells[1]).replace(/[^0-9]/g,'');const qty=Math.max(1,Number(qtyText||1));addParsed(cells[0],qty);if(!findItem(cells[0]))unmatched.push(cells[0]);continue}const rangeItems=expandPastedItemRange(line);if(rangeItems.length){rangeItems.forEach(item=>{addParsed(item,1);if(!findItem(item))unmatched.push(item)});continue}const parsedLine=parsePastedItemLine(line);if(!findItem(parsedLine.item)&&!/\d/.test(parsedLine.item))continue;addParsed(parsedLine.item,parsedLine.qty);if(!findItem(parsedLine.item))unmatched.push(parsedLine.item)}if(fields.customer){const matched=customers.find(c=>normalizeCustomer(c.business_name)===normalizeCustomer(fields.customer));if(matched){document.querySelector('input[name="proxyCustomerMode"][value="select"]').checked=true;$('proxyCustomer').value=matched.id}else{document.querySelector('input[name="proxyCustomerMode"][value="direct"]').checked=true;$('proxyDirectName').value=fields.customer}updateCustomerMode();reloadSelectedCustomerPrices();loadProxyDestinations()}if(fields.delivery)$('proxyDeliveryName').value=fields.delivery;if(fields.phone)$('proxyDeliveryPhone').value=fields.phone;if(fields.address)$('proxyDeliveryAddress').value=fields.address;if(fields.memo)$('proxyMemo').value=fields.memo;const parsed=[...merged.values()];if(parsed.length){$('proxyLines').innerHTML='';parsed.forEach(row=>addLine(row));refreshAllLinePrices()}$('proxyPasteInput').value='';const result=$('proxyPasteResult');if(result)result.textContent=`주문정보 적용 완료 · ${parsed.length.toLocaleString()}품번${unmatched.length?` · 상품 미등록 확인 ${[...new Set(unmatched)].length}개`:''}`;calc();scheduleProxyDraftSave()}

async function loadProxyDestinations(){const customerId=selectedCustomerId(),select=$('proxyDeliverySelect');if(!select)return;select.innerHTML='<option value="registered">등록 주소 사용</option><option value="new">+ 새 납품처 입력</option>';if(!customerId)return;const customer=customers.find(c=>String(c.id)===String(customerId));const {data}=await supabaseClient.from('customer_delivery_destinations').select('id,delivery_name,delivery_phone,delivery_address,is_default,last_used_at').eq('customer_id',customerId).order('is_default',{ascending:false}).order('last_used_at',{ascending:false}).limit(100);const destinations=data||[];select.innerHTML='<option value="registered">등록 주소 사용</option>'+destinations.map(row=>`<option value="${row.id}">${esc(row.delivery_name)}${row.is_default?' · 기본':''}</option>`).join('')+'<option value="new">+ 새 납품처 입력</option>';select.onchange=()=>{const row=destinations.find(x=>String(x.id)===select.value);if(select.value==='registered'){ $('proxyDeliveryName').value=customer?.business_name||'';$('proxyDeliveryPhone').value=customer?.phone||'';$('proxyDeliveryAddress').value=customer?.address||'';}else if(select.value==='new'){$('proxyDeliveryName').value='';$('proxyDeliveryPhone').value='';$('proxyDeliveryAddress').value='';}else if(row){$('proxyDeliveryName').value=row.delivery_name||'';$('proxyDeliveryPhone').value=row.delivery_phone||'';$('proxyDeliveryAddress').value=row.delivery_address||'';}scheduleProxyDraftSave()};}
