...
$('proxyCustomerSearch')?.addEventListener('change',event=>{const query=String(event.target.value||'').trim(),exact=directCustomerRows().find(row=>normalizeCustomer(row.name)===normalizeCustomer(query)),matches=directCustomerRows().filter(row=>lookupMatches(row.name,query));if(exact)selectUnifiedProxyParty(exact);else if(query){$('proxyDirectName').value=query;document.querySelector('input[name="proxyCustomerMode"][value="direct"]').checked=true;$('proxySelectedCustomerLabel').textContent=`새 미가입 거래처: ${query}`;loadProxyDestinations();reloadSelectedCustomerPrices();calc()}});
