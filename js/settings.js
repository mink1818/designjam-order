...
async function loadAdminAccounts(){
  ...
  let health=document.getElementById('adminPresenceHealth');if(!health){health=document.createElement('p');health.id='adminPresenceHealth';health.className='account-admin-note';health.setAttribute('aria-live','polite');box.parentElement?.insertBefore(health,box)}
  box.innerHTML='<p>관리자 계정을 불러오는 중...</p>';
  const {data:{user}}=await supabaseClient.auth.getUser();
  let data=null,error=null;
  const presenceResult=await supabaseClient.rpc('get_admin_presence');
  ...
  const now=Date.now();
  const statusOf=a=>{
    const seen=a.admin_last_seen_at?new Date(a.admin_last_seen_at).getTime():0;
    const loggedOut=a.admin_logged_out_at?new Date(a.admin_logged_out_at).getTime():0;
    if(!seen||loggedOut>=seen)return {key:'offline',label:'오프라인'};
    const minutes=(now-seen)/60000;
    if(minutes<=2)return {key:'online',label:'접속 중'};
    if(minutes<=10)return {key:'away',label:'자리 비움'};
    return {key:'offline',label:'오프라인'};
  };
  const dateText=value=>value?new Date(value).toLocaleString('ko-KR'):'기록 없음';
  box.innerHTML=rows.length?rows.map(a=>{
    const name=a.business_name||a.owner_name||'관리자';
...
async function changeAdminRole(id,role,select){
  try{await window.requireAdminSecurity?.('email');}catch(error){loadAdminAccounts();return alert(error.message||error);}
  const label=role==='developer_admin'?'개발관리자':role==='manager'?'매니저':'관리자';if(!confirm(`이 계정 권한을 ${label}로 변경할까요?`)){loadAdminAccounts();return;}
...
let adminAccountsLoading=false,adminActivityLoading=false;
const loadAdminAccountsOnce=loadAdminAccounts;
loadAdminAccounts=async function(){if(adminAccountsLoading||document.visibilityState==='hidden')return;adminAccountsLoading=true;try{return await loadAdminAccountsOnce()}finally{adminAccountsLoading=false}};
async function loadAdminActivity(){const box=document.getElementById('adminActivityList');if(!box||adminActivityLoading||document.visibilityState==='hidden')return;adminActivityLoading=true;try{const {data,error}=await supabaseClient.rpc('get_admin_activity',{p_limit:50});if(error){box.innerHTML=`<p>활동기록을 불러오지 못했습니다: ${esc(error.message)} · V6.5.89 SQL을 실행해주세요.</p>`;return}const rows=data||[];box.innerHTML=rows.length?`<div class="admin-activity-list">${rows.map(row=>`<article><strong>${esc(row.admin_name||'관리자')} · ${esc(row.admin_role==='manager'?'매니저':row.admin_role==='developer_admin'?'개발관리자':'관리자')}</strong><span>${new Date(row.created_at).toLocaleString('ko-KR')} · 활동</span><small>${esc(row.page_path||'-')} · ${esc(row.device_info||'-')}</small></article>`).join('')}</div>`:'<p>저장된 활동기록이 없습니다.</p>'}finally{adminActivityLoading=false}}
document.addEventListener('DOMContentLoaded',()=>{document.getElementById('createAdminBtn')?.addEventListener('click',createAdminAccount);setTimeout(()=>{loadAdminAccounts();loadAdminActivity()},300);setInterval(()=>{if(document.visibilityState==='visible')loadAdminAccounts()},120000);setInterval(()=>{if(document.visibilityState==='visible')loadAdminActivity()},300000);});
