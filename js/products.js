...
async function importCustomerItemPricesFromGrid() {
  ...
  for(let i=0;i<namePayload.length;i+=500){const batch=namePayload.slice(i,i+500);const {data,error}=await supabaseClient.rpc("upsert_customer_name_item_prices",{p_prices:batch});if(error)throw new Error(`직접입력 거래처 단가 저장 실패: ${error.message}. V6.5.6 SQL을 먼저 실행해주세요.`);nameSaved+=Number(data?.saved??batch.length);}
