(() => {
  ...
  function ensureStyles() {
    ...
    style.textContent = `
      #designjamSessionStatus.session-status{position:fixed!important;top:10px!important;right:10px!important;left:auto!important;z-index:2147483647!important;display:flex!important;visibility:visible!important;opacity:1!important;align-items:center!important;gap:8px!important;max-width:calc(100vw - 20px)!important;padding:8px 9px 8px 11px!important;border:1px solid #31577f!important;border-radius:14px!important;background:#173a63!important;background-color:#173a63!important;backdrop-filter:none!important;-webkit-backdrop-filter:none!important;color:#fff!important;box-shadow:0 6px 18px rgba(0,0,0,.16)!important;font-family:Arial,sans-serif!important;transform:none!important}
      #designjamSessionStatus.session-status-customer{background:#24589f!important;background-color:#24589f!important}
      ...
      #designjamSessionStatus .session-status-text span{max-width:180px!important;overflow:hidden!important;color:#e8f2ff!important;font-size:11px!important;text-overflow:ellipsis!important;white-space:nowrap!important}
      #designjamSessionStatus .session-status-action{flex:0 0 auto!important;padding:7px 9px!important;border:0!important;border-radius:9px!important;background:#ffffff!important;background-color:#ffffff!important;color:#173a63!important;font-size:12px!important;font-weight:700!important;cursor:pointer!important}
      ...
      #designjamSessionStatus{opacity:1!important;filter:none!important;mix-blend-mode:normal!important;isolation:isolate!important}#designjamSessionStatus.session-status{background-image:none!important}#designjamSessionStatus .session-status-action{opacity:1!important;background-image:none!important}
      ...
      #designjamDesktopBrowserControls{position:fixed;top:10px;left:132px;z-index:2147483000;display:flex;align-items:center;gap:6px;padding:0;background:transparent;font-family:inherit}#designjamDesktopBrowserControls button{min-width:0;padding:7px 11px;border:1px solid rgba(255,255,255,.45);border-radius:999px;background:rgba(14,52,105,.92);color:#fff;font-family:inherit;font-size:13px;line-height:1.2;font-weight:800;box-shadow:0 4px 14px rgba(0,0,0,.18);backdrop-filter:blur(6px);-webkit-backdrop-filter:blur(6px);cursor:pointer}#designjamDesktopBrowserControls button:hover{background:#0f3f7d}#designjamDesktopBrowserControls button:active{transform:translateY(1px)}
      ...
      @media print{#designjamSessionStatus.session-status,#designjamSessionStatus,#designjamNotificationPanel{display:none!important;visibility:hidden!important}}
      @media(max-width:899px){#designjamDesktopBrowserControls{display:none!important}}
    `;
  ...
  function createStatusBar({ role, title, detail }) {
    ...
    box.innerHTML = `
      ...
      <button type="button" class="session-status-action session-notification-button" aria-label="알림 열기">🔔<span class="session-notification-count">0</span></button><button type="button" class="session-status-action session-status-logout">로그아웃</button>
    `;
...
(function addDesktopHomeShortcut(){
  function mount(){
    ...
    link.innerHTML = '<span class="desktop-home-arrow" aria-hidden="true">◀</span><span>디자인 삭스</span>';
...
// V6.5.56: 관리자·거래처 PC 화면 공통 브라우저 이동 버튼
(function addDesktopBrowserControls(){
  function mount(){
    if (document.getElementById('designjamDesktopBrowserControls')) return;
    const page = document.body?.dataset?.sessionPage;
    if (!page || window.matchMedia('(max-width: 899px)').matches) return;
    const current = location.pathname.split('/').pop() || 'index.html';
    if (current === 'statement.html') return;
    const controls = document.createElement('nav');
    controls.id = 'designjamDesktopBrowserControls';
    controls.setAttribute('aria-label', '페이지 이동');
    controls.innerHTML = `
      <button type="button" data-browser-action="back" aria-label="이전 페이지">← 뒤로</button>
      <button type="button" data-browser-action="forward" aria-label="다음 페이지">앞으로 →</button>
      <button type="button" data-browser-action="refresh" aria-label="현재 페이지 새로고침">↻ 새로고침</button>`;
