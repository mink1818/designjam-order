...
try {
  // V6.6.68: 주문 자체를 먼저 받고, 현재 화면에 필요한 고객/표시 데이터만 병렬 조회합니다.
...
async function saveUnpaidInlineMemo(input,orderNumber,customerId,customerName,total){
  ...
  input.value=String(data?.memo||memo||'');document.querySelectorAll('[data-unpaid-memo-order]').forEach(el=>{if(el!==input&&el.dataset.unpaidMemoOrder===String(orderNumber))el.value=input.value;});document.querySelectorAll('[data-unpaid-memo-display]').forEach(el=>{if(el.dataset.unpaidMemoDisplay===String(orderNumber)){el.textContent=input.value?`메모 · ${input.value}`:'';el.hidden=!input.value;el.closest('.payment-memo-combo')?.classList.toggle('has-unpaid-memo',Boolean(input.value));}});input.classList.add('saved');setTimeout(()=>input.classList.remove('saved'),900);
}
...
>
  ...
  <div class="mobile-order-summary" aria-label="주문 요약">
    ...
    ${paymentTracked?`<div class="payment-memo-combo mobile-payment-combo ${paymentRecord.memo?'has-unpaid-memo':''}" onclick="event.stopPropagation()"><label class="order-payment-check mobile-payment-check ${paymentStatus==='입금완료'?'paid':paymentStatus==='일부입금'?'partial':''}"><input type="checkbox" ${paymentStatus==='입금완료'?'checked':''} onchange="toggleOrderPaid(this,'${escapeAdminAttr(group.orderNumber)}','${escapeAdminAttr(group.customerId||'')}',${summaryTotal},'${escapeAdminAttr(group.customerName||'')}',${index})"><span>${paymentStatus==='입금완료'?'입금':paymentStatus==='일부입금'?`일부 ${paidAmount.toLocaleString()}원`:'미입금'}</span>${paymentRecord.updated_at?`<small>${escapeAdminHtml(paymentRecord.confirmed_by_name||'관리자')} · ${formatHourMinute(paymentRecord.updated_at)}</small>`:''}</label><span class="collapsed-unpaid-memo" data-unpaid-memo-display="${escapeAdminAttr(group.orderNumber)}" ${paymentRecord.memo?'':'hidden'}>메모 · ${escapeAdminHtml(paymentRecord.memo||'')}</span></div>`:''}
  </div>
  <div class="order-status-stack">
    ${paymentTracked?`<div class="payment-memo-combo desktop-payment-combo ${paymentRecord.memo?'has-unpaid-memo':''}" onclick="event.stopPropagation()"><label class="order-payment-check desktop-payment-check ${paymentStatus==='입금완료'?'paid':paymentStatus==='일부입금'?'partial':''}"><input type="checkbox" ${paymentStatus==='입금완료'?'checked':''} onchange="toggleOrderPaid(this,'${escapeAdminAttr(group.orderNumber)}','${escapeAdminAttr(group.customerId||'')}',${summaryTotal},'${escapeAdminAttr(group.customerName||'')}',${index})"><span>${paymentStatus==='입금완료'?'입금':paymentStatus==='일부입금'?`일부 ${paidAmount.toLocaleString()}원`:'미입금'}</span>${paymentRecord.updated_at?`<small>${escapeAdminHtml(paymentRecord.confirmed_by_name||'관리자')} · ${formatHourMinute(paymentRecord.updated_at)}</small>`:''}</label><span class="collapsed-unpaid-memo" data-unpaid-memo-display="${escapeAdminAttr(group.orderNumber)}" ${paymentRecord.memo?'':'hidden'}>메모 · ${escapeAdminHtml(paymentRecord.memo||'')}</span></div>`:''}
