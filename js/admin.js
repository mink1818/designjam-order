...
>
  ...
  <div class="order-status-stack">
    <div class="customer-notify-order-actions" onclick="event.stopPropagation()"><label class="customer-notify-select-wrap"><input class="customer-notify-select" type="checkbox" data-customer-id="${escapeAdminAttr(group.customerId||'')}" data-customer-name="${escapeAdminAttr(group.customerName||'')}" data-order-number="${escapeAdminAttr(group.orderNumber||'')}" onchange="toggleCustomerNotifySelection(this)"> 선택</label><button type="button" class="customer-notify-btn" onclick="openCustomerNotification('${escapeAdminAttr(group.orderNumber||'')}','${escapeAdminAttr(group.customerId||'')}','${escapeAdminAttr(group.customerName||'')}')">🔔 거래처 알림</button></div>
