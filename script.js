...
function renderProducts(categoryName) {
  ...
  backBtn.className = "cart-btn";
  backBtn.textContent = "← 카테고리로 돌아가기";
  ...
  if (products.length === 0) {
    productList.innerHTML += `
      <div class="product-card">
        ...
        <p>이 카테고리에 상품 사진을 추가하면 여기에 표시됩니다.</p>
      </div>
    `;
    return;
  }
...
function openOrder(product) {
  ...
  const backBtn = document.createElement("button");
  backBtn.className = "cart-btn";
  backBtn.textContent = "← 상품 사진 목록으로 돌아가기";
  backBtn.addEventListener("click", function () {
  ...
  productList.innerHTML += `
    <div class="product-card">
      ...
      ${inputHtml}
      <button class="cart-btn" id="addCartBtn">장바구니 담기</button>
    </div>
  `;
...
function addToCart(product) {
  ...
  alert(`장바구니에 ${addedCount}개 담겼습니다.`);
  renderCart();
}
